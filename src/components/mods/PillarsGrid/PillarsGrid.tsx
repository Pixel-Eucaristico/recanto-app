...
/**
 * PillarsGrid - Grade de pilares/features com ícones
    ...
    <section className={`w-full py-20 px-6 ${bgColorClasses[bgColor]} ${maxWidthClasses[maxWidth]} mx-auto`}>
      ...
      <div className={`grid grid-cols-1 ${columnClasses[columns]} gap-8`}>
        {pillars.map((pillar, index) => {
          ...
          // Debug: log se ícone não for encontrado
          if (!(LucideIcons as any)[iconName]) {
            console.warn(`⚠️ [PillarsGrid] Ícone "${pillar.icon}" (mapeado para "${iconName}") não encontrado no Lucide.`, {
              iconOriginal: pillar.icon,
              iconMapeado: iconName,
          ...
          return (
            <motion.div
              ...
              viewport={{ once: true }}
              className="card bg-base-100 shadow-xl p-6 items-center text-center hover:shadow-2xl transition-shadow"
            >
              <div className="mb-4">
                <IconComponent
                  size={40}
              ...
              <p className="text-sm text-base-content/80">{pillar.description}</p>
            </motion.div>
          );
        })}
      </div>
