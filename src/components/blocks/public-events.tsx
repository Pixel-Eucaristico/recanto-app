 ...
 * Displays upcoming public events on the homepage
 */
...
export default function PublicEvents() {
  ...
  useEffect(() => {
    const loadPublicEvents = async () => {
      try {
        const publicEvents = await eventService.getPublicEvents(5);
  ...
  if (events.length === 0) {
    return null; // Don't show section if no public events
  }
      ...
      <div className="container mx-auto px-4">
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-slate-600 max-w-2xl mx-auto">
            Venha participar dos nossos encontros comunitários de oração, formação e celebração
          </p>
        </div>
