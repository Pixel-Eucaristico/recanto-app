...
export class TrackEnrollmentRepository extends BaseRepository<TrackEnrollmentRequest> {
  ...
  /**
   * Matrículas APROVADAS das trilhas — a população real de alunos.
   *
   * A lista de alunos era derivada de `formation_progress`, então quem foi aprovado
   * e nunca abriu uma aula não existia na tela. É justamente quem o formador mais
   * precisa ver.
   */
  async findApprovedByTracks(trackIds: string[]): Promise<TrackEnrollmentRequest[]> {
    if (trackIds.length === 0) return [];
    const batches: string[][] = [];
    for (let i = 0; i < trackIds.length; i += 30) batches.push(trackIds.slice(i, i + 30));
