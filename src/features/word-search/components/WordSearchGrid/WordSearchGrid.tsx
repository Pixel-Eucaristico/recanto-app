...
/** Gera todas as células entre start e end se estiverem em linha reta (horiz/vert/diag). */
function lineCells(start: Cell, end: Cell): Cell[] | null {
  const dr = end.row - start.row;
  const dc = end.col - start.col;
  const steps = Math.max(Math.abs(dr), Math.abs(dc));
  if (steps === 0) return [start];
  // precisa ser linha reta (horizontal, vertical ou diagonal exata)
...
export function WordSearchGrid({
  ...
  function isCellInPreview(row: number, col: number): boolean {
    ...
    return preview.some(c => c.row === row && c.col === col);
  }
  ...
  function updateSelect(row: number, col: number) {
    if (!dragging) return;
    setCurrentCell({ row, col });
  }
        ...
        <div className={`card ${allFound ? 'bg-success/10 border-success' : 'bg-base-100 border-base-300'} border`}>
          <div className="card-body items-center text-center gap-2">
            ...
            <h2 className="text-2xl font-bold text-base-content">
              {allFound ? 'Parabéns!' : 'Resultado'}
            </h2>
            <p className="text-base-content/70">
              {found.length} de {puzzle.words.length} palavras ({result.score.toFixed(1)}%)
            </p>
            <p className="text-xs text-base-content/50">
              {result.persisted
                ? 'Primeira tentativa — registrada para o formador.'
                : 'Retake — score mostrado, não registrado.'}
            </p>
            {onRestart && (
              <button className="btn btn-ghost btn-sm gap-1 mt-2" onClick={reset}>
        ...
        <div className="inline-block">
          <div
            ...
            onTouchMove={e => {
              const t = e.touches[0];
              if (!t) return;
              const c = cellAtPoint(t.clientX, t.clientY);
              if (c) updateSelect(c.row, c.col);
            }}
