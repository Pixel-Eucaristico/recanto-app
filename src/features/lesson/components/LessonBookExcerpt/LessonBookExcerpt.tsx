    ...
    Promise.all([
      ...
      .then(([book, chapters]) => {
        ...
        const renumbered = sortedChapters.map(ch => {
          if (BookEntity.kindOf(ch) === 'chapter') {
            bodyCounter++;
            return BookEntity.numberChapter(ch, bodyCounter);
          }
          return BookEntity.numberChapter(ch);
        });
        const sliced = BookEntity.sliceByRange(renumbered, citation.start_ref, citation.end_ref);
        ...
        const chaptersInRange = renumbered
          ...
          .filter(c => {
            ...
            if (hasExplicitRange) {
              return (c.chapter.kind ?? 'chapter') === 'chapter';
            }
            return true;
          });
